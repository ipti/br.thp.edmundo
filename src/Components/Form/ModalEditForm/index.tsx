...
const ModalEditForm = ({ visible, onHide }: { visible: Question, onHide(): void }) => {
    ...
    useEffect(() => {
        setOptions(visible?.options?.map((item) => {
            return {
                id: item.id,
                content: item.content,
                questionId: item.questionId,
                response_question: visible.response_question.find((res) => res.option_fk === item.id) ? true : false
            }
        }))
    }, [visible])
    ...
    return (
        <Dialog visible={!!visible} onHide={onHide} header="Editar questão">
            <Formik initialValues={{ content: visible?.content ?? "", question: visible?.options ?? [], }} onSubmit={(values) => {
                contextEditActivities?.handleQuestionUpdate({ content: values.content, id: visible.id, options: options })
                onHide()
            }}>
                                ...
                                options?.map((item, key) => {
                                    return (
                                        <Padding padding="8px 0" >
                                            {visible.type === "SELECTION_BOX" && <Row key={key}>
                                                <Column id="center">
                                                    <CheckboxComponent
                                                        value={item?.response_question} checked={item?.response_question === true} onChange={() => {
                                                            const newForm = [...options]
                                                            newForm[key].response_question = !newForm[key].response_question
