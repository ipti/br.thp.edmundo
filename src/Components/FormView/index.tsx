...
const FormViewComponent = ({ form, values }: { form: FormView, values?: CreateResponse}) => {

    return (
        <div>
            <Column style={{ gap: 32 }}> 
                {form?.answer_form![0]?.answer_question?.map((item: any, key: number) => {
                    return (
                        <Padding padding="0" key={key}>
                            <p>{item.question.content}</p>
                            <Padding padding="8px" />
                            {item.question.type === "MULTIPLE_CHOICE" && <RenderRadioButtonCard question={item}/>}
                            {item.question.type === "SELECTION_BOX" && <RenderCheckBoxCard question={item}/>}
